(() => {
 ...
 let session=null, pending=false, refreshTimer, pollTimer;
 ...
 async function library() {
   ...
   say(mode+mp+(data.unlocked?'Sua coleção está liberada. Escolha uma história.':'Sua coleção ainda não foi liberada. Se acabou de pagar, aguarde a confirmação e verifique novamente.'));
   if(data.unlocked) for(const book of window.PIRILUME_BOOKS.filter(b=>!b.free)) {
     ...
     button.onclick=()=>run(async()=>{const full=await api('/book/'+book.id); Object.assign(book,full); let option=[...$('book-picker').options].find(o=>o.value===book.id); if(!option){option=document.createElement('option');option.value=book.id;option.textContent=book.title;$('book-picker').append(option);} location.hash='#livro/'+book.id;});
 ...
 function logout(){clearTimeout(refreshTimer);clearTimeout(pollTimer);setSession(null);$('owned-books').replaceChildren();for(const b of window.PIRILUME_BOOKS.filter(b=>!b.free)){delete b.chapters;delete b.cover;}[...$('book-picker').options].filter(o=>o.value!=='luzes').forEach(o=>o.remove());location.hash='#biblioteca';say('Você saiu da sua conta.');}
 async function restoreSession(){const stored=loadStoredSession();if(!stored?.refresh_token)return;try{setSession(await auth('token?grant_type=refresh_token',{refresh_token:stored.refresh_token}));await library();}catch{setSession(null);say('Sua sessão expirou. Entre novamente.');}}
