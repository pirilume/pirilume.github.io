(() => {
 ...
 async function restoreSession(){const stored=loadStoredSession();if(!stored?.refresh_token)return;try{setSession(await auth('token?grant_type=refresh_token',{refresh_token:stored.refresh_token}));await library();}catch{clearStoredSession();}}
